...
# License: BSD
# Modified: 2025 by katoy & ChatGPT (minimal diff, macOS + Matplotlib 3.9)
# ----------------------------------------------------------------------------
import matplotlib
matplotlib.use("macosx")  # macOS GUIバックエンドを明示指定
...
def text3d(ax, xyz, s, zdir="z", size=0.1, angle=0, **kwargs):
    ...
    # 修正①: 標準フォント "DejaVu Sans" を使用
    path = TextPath((0, 0), s, size=size,
...
maxrange = max(xrange[1] - xrange[0],
               yrange[1] - yrange[0],
               zrange[1] - zrange[0])
...
for x in np.arange(xrange[0], xrange[1] + xticks[1], xticks[1]):
    x = xtransform(x)
    ax.plot([x, x, x], [ymin, ymax, ymax], [zmin, zmin, zmax],
            linewidth=linewidth, color=color, zorder=-50)
for y in np.arange(yrange[0], yrange[1] + yticks[1], yticks[1]):
    y = ytransform(y)
    ax.plot([xmax, xmin, xmin], [y, y, y], [zmin, zmin, zmax],
            linewidth=linewidth, color=color, zorder=-50)
for z in np.arange(zrange[0], zrange[1] + zticks[1], zticks[1]):
    z = ztransform(z)
    ax.plot([xmax, xmin, xmin], [ymax, ymax, ymin], [z, z, z],
            linewidth=linewidth, color=color, zorder=-50)
...
for x in np.arange(xrange[0], xrange[1] + xticks[0], xticks[0]):
    x = xtransform(x)
    ax.plot([x, x, x], [ymin, ymax, ymax], [zmin, zmin, zmax],
            linewidth=linewidth, color=color, zorder=-25)
...
ax.plot([xmin, xmin, xmax, xmax, xmin],
        ...
        linewidth=linewidth, color=color, zorder=-10)
ax.plot([xmin, xmin, xmax, xmax, xmin],
        [ymax, ymax, ymax, ymax, ymax],
        [zmin, zmax, zmax, zmin, zmin],
        linewidth=linewidth, color=color, zorder=-10)
ax.plot([xmin, xmin, xmin, xmin, xmin],
...
for x in np.arange(xrange[0], xrange[1] + xticks[1], xticks[1]):
    x = xtransform(x)
    ax.plot([x, x], [ymin, ymin - length], [zmin, zmin],
            linewidth=linewidth, color=color, zorder=-50)
for y in np.arange(yrange[0], yrange[1] + yticks[1], yticks[1]):
    y = ytransform(y)
    ax.plot([xmax, xmax + length], [y, y], [zmin, zmin],
            linewidth=linewidth, color=color, zorder=-50)
for z in np.arange(zrange[0], zrange[1] + zticks[1], zticks[1]):
    z = ztransform(z)
    ax.plot([xmin, xmin - length], [ymin, ymin], [z, z],
            linewidth=linewidth, color=color, zorder=-50)
...
for x in np.arange(xrange[0], xrange[1] + xticks[0], xticks[0]):
    x_ = xtransform(x)
    ax.plot([x_, x_], [ymin, ymin - length], [zmin, zmin],
            linewidth=linewidth, color=color, zorder=-50)
    text3d(ax, (x_, ymin - 2 * length, zmin),
           "%.1f" % x,
           size=0.04,
           facecolor="black",
           edgecolor="None")
...
for y in np.arange(yrange[0], yrange[1] + yticks[0], yticks[0]):
    y_ = ytransform(y)
    ax.plot([xmax, xmax + length], [y_, y_], [zmin, zmin],
            linewidth=linewidth, color=color, zorder=-50)
    text3d(ax, (xmax + 2 * length, y_, zmin),
           "%.1f" % y,
           angle=np.pi / 2,
           size=0.04,
           facecolor="black",
...
for z in np.arange(zrange[0], zrange[1] + zticks[1], zticks[0]):
    z_ = ztransform(z)
    ax.plot([xmin, xmin - length], [ymin, ymin], [z_, z_],
            linewidth=linewidth, color=color, zorder=-50)
    text3d(ax, (xmin - 2 * length, ymin, z_),
           "%.1f" % z,
           zdir="y",
           size=0.04,
           facecolor="black",
...
ax.plot_wireframe(X, Y, Z, rstride=1, cstride=1, color="black",
                  linewidth=5, antialiased=True, zorder=10)
ax.plot_wireframe(X, Y, Z, rstride=1, cstride=1, color="white",
                  linewidth=2, antialiased=True, zorder=20)
ax.plot_surface(X, Y, Z, rstride=5, cstride=5, facecolors=C,
                shade=False, edgecolor="None", linewidth=0,
                antialiased=True, zorder=30)

text3d(ax, ((xmin + xmax) / 2, ymax, 1.05 * zmax),
       "X axis", zdir="y", size=0.05,
       facecolor="black", edgecolor="None")
text3d(ax, (xmin, (ymin + ymax) / 2, 1.05 * zmax),
       "Y axis", zdir="x", size=0.05,
       facecolor="black", edgecolor="None")
text3d(ax, (1.05 * xmax, ymax, (zmin + zmax) / 2),
       "Z axis", angle=np.pi / 2, zdir="y", size=0.05,
       facecolor="black", edgecolor="None")

plt.tight_layout()
plt.show(block=True)  # 修正③: macOSで確実にウィンドウを保持
